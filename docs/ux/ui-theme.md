...
- 베이스 컴포넌트: shadcn/ui
- 테마 생성 도구: tweakcn
- 용도: 자영업 HR SaaS 대시보드용, 밝고 신뢰감 있는 느낌

## 2. 브랜드 키워드
- 키워드: 신뢰, 데이터 기반, 안정감, 따뜻한 상생
- 톤 앤 매너:
  - 존댓말, 차분하지만 너무 딱딱하지 않게
  - “경영 도우미” 느낌
...
아래 CSS 변수들은 **실제 웹 앱**의 공통 스타일 파일
(예: `apps/web/src/styles/theme.css`)에도 그대로 정의해서 사용한다.
